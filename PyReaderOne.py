...
reader = PdfReader("sandisk1.pdf")
sandisk_szablon = pd.read_excel('sandisk1.xlsx', dtype={'SO Line':str})
...
# ilosć usunąć przecinek
        ...
        if len(kraj)>=2:
            if iterator_pomocniczy % 2==0:
                ...
                waga = str(waga[1])
                waga = waga.replace('.',',')
                tablica_danych.append([waga,ilosc[1],kraj[0], "Połącz kropki", f'Strona {page_number+1}'])
                ...
                waga = str(waga)
                waga = waga.replace('.',',')
                tablica_danych.append([waga,ilosc[1],"", "Połącz kropki", f'Strona {page_number+1}'])
                iterator_pomocniczy +=1
            ...
            waga = str(waga[1])
            waga = waga.replace('.',',')
            tablica_danych.append([waga,ilosc[1],kraj[0]])
            # ========= DZIAŁA START ===========
...
# print(sandisk_szablon)


df_dane = pd.DataFrame(tablica_danych)
# print(df_dane.columns)
df_nowa = pd.DataFrame()
df_nowa['Weight [kg]'] = df_dane[0]
df_nowa['COO Qty'] = df_dane[1]
df_nowa['COO'] = df_dane[2]
